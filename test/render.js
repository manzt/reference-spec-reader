...
test("Simple string/number substitution", () => {
	[
		...
		["{{  i }}", { i: "text" }, "text"],
		[
			`{{ aLongVariableName }}.png`,
			{ aLongVariableName: "hello-text" },
			"hello-text.png",
		],
...
test("Function with named arguments", () => {
	[
		...
		[
			`http://{{ f(c='text', f = 199, foo ="bar") }}`,
			{ f: ({ c, f, foo }) => `${c}_${f}_${foo}` },
			"http://text_199_bar",
		],
		[
			`http://{{ f(c='text', f = 199, foo ="bar"), }}`,
		...
		[
			`http://{{some_long_function_name(foo='bar',i=10)}}`,
			{ some_long_function_name: ({ foo, i }) => `${foo}_${i}` },
			"http://bar_10",
		],
	].forEach(([template, ctx, expected]) => {
		assert.is(render(template, ctx), expected);
